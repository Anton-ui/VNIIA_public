...
{
    face_cascade.detectMultiScale(mat_vector, faces, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE, Size(10, 10));
    for (v = 0; v < faces.size(); ++v)
    {
        scene_corners[0] = cvPoint(faces[v].x,                  faces[v].y);
        scene_corners[1] = cvPoint(faces[v].x,                  faces[v].y + faces[v].height);
        scene_corners[2] = cvPoint(faces[v].x + faces[v].width, faces[v].y + faces[v].height);
        scene_corners[3] = cvPoint(faces[v].x + faces[v].width, faces[v].y);
